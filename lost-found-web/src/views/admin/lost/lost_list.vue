  ...
  <div class="lf_container">
    <a-modal v-model:open="data.modalVisible" :title="data.updateID === 0 ? '添加物品' : '编辑物品'" @ok="handleOk">
      <a-form :model="formState" name="basic" ref="formRef" :label-col="{ span: 4 }" :wrapper-col="{ span: 20 }"
        autocomplete="off">
        <a-form-item label="丢失物品名" name="lost_name" has-feedback
          :rules="[{ required: true, message: '请输入物品名', trigger: 'blur' }]">
          <a-input v-model:value="formState.lost_name" placeholder="物品名" />
        </a-form-item>
        <a-form-item label="丢失地点" name="lost_place" has-feedback
          :rules="[{ required: true, message: '请输入丢失地点', trigger: 'blur' }]">
          <a-input v-model:value="formState.lost_place" placeholder="丢失地点" />
        </a-form-item>


        <a-form-item label="丢失时间" name="lost_time" has-feedback
          :rules="[{ required: true, message: '请输入丢失时间', trigger: 'blur' }]">
          <a-input-password v-model:value="formState.lost_time" placeholder="丢失时间" />
        </a-form-item>

        <a-form-item label="物品图" name="lost_picture" has-feedback
          :rules="[{ required: true, message: '请输入物品图' ,  trigger: 'blur' }]">
          <a-input-password v-model:value="formState.lost_picture" placeholder="物品图" />
        </a-form-item>

        <a-form-item label="信息描述" name="lost_describe"
          :rules="[{ required: true, message: '请输入信息描述', trigger: 'blur' }]">
          <a-input-password v-model:value="formState.lost_describe" placeholder="信息描述" />
      ...
      <a-spin :spinning="data.spinning" tip="加载中..." :delay="300">
        <a-table :columns="data.columns" :pagination="false" :data-source="data.list"
          ...
          <template #bodyCell="{ column, record }">
            <template v-if="column.key === 'lost_picture'">
              <img class="lf_user_avatar" :src="record.lost_picture" alt="">
            </template>
...
const data = reactive({
  ...
  columns: [
    ...
    { title: '丢失物品名', dataIndex: 'lost_name', key: 'lost_name' },
    {
      title: '丢失地点',
    ...
    {
      title: '物品图',
      dataIndex: 'lost_picture',
      key: 'lost_picture',
    },
    {
      title: '信息描述',
      key: 'lost_describe',
      dataIndex: 'lost_describe',
    },
    {
      title: '状态',
      key: 'status',
  ...
  list: [
    {
      ...
      lost_name: "苹果15",
      lost_place: '中原区',
      lost_time: "2024--06--18",
      lost_picture: "https://www.apple.com.cn/iphone-15/images/overview/portrait/portrait_mode__wzbojol1j2a6_large_2x.jpg",
      lost_describe: "奶奶滴真倒霉",
      status: 1,
    }
  ],
  count: 20,
