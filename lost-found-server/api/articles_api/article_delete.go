...
type ArticleRemoveResponse struct {
...
func (ArticleApi) ArticleDeleteView(c *gin.Context) {
	...
	var resList []ArticleRemoveResponse
	var a int64
	//rq.IDList是我们传入要删除的图片id，在删除前判断是否有权限删除
	// 遍历物品ID数组
	for _, itemId := range rq.IDList {
		var item models.ArticleModel
		var response ArticleRemoveResponse
		// 查询物品以获取其用户ID
		...
		if item.UserID != uintID {
			...
			response.ID = strconv.Itoa(int(itemId))
			response.IsSuccess = false
			response.Msg = "你没有权限删除此物品"
			// 如果查询失败，可能是物品不存在
			resList = append(resList, response)
			continue
		}
		//这是要删除的id
		var imagelist []models.ArticleModel
		//获取要删除的数据到imagelist中
	...
	res.Ok(resList, fmt.Sprintf("共删除%d张物品", a), c)
}
